...
本项目是为了在本电脑上实现https://github.com/jianyuan1991/ragdemo，由于我们本来就使用了Ruo-Yi的开源，在此配置的基础上实现github中的内容

github的作者之前写过一篇[Spring AI+Ollama本地环境搭建](https://www.bxmdm.com/archives/2024030801)的文章，在这里的环境改动是jdk的升级（1.8->1.17），maven的降级（最新版->3.8.4）。

## RAG简介
RAG是目前大模型应用落地的一套解决方案，中文名叫检索增强，由于大语言模型有时效性和幻觉等局限性，使用RAG方案，先利用搜索技术从本地知识中搜索出想要的相关信息，在将相关信息组成prompt中上下文的一部分，在由大模型根据prompt进行回复。本次就构建一个demo，使用RAG技术构建一个文档问答的应用。如图所示，RAG文档问答的整体流程大致分成两个阶段：
...
要使用pgvector本机是需要安装postgresql的，但是由于本机的5432端口被占了，这里我该成5433了，具体的配置查看工程文件中的`src/main/resources/application.yml`（还有用户名密码等自行配置即可）

进入**postgresql**的命令行：
`psql -h localhost -p 5433 -U postgres`

由于本人不太会用docker，所以参考官方文档的方法在windows上直接下载插件（==官方文档就是好啊==）
...
```
set "PGROOT=C:\Program Files\PostgreSQL\16"//自己下载的Postgresql路径
cd %TEMP%
...
这里修改了代码，不然会报400，Bad request，路径是src/main/java/com/bxmdm/ragdemo/controller/IndexController.java。

而且在使用postman的时候==需要填写key值File==
![IndaexController的修改](./img/IndaexController的修改.jpg)

上传的文件在D盘中
![储存文件的地方](./img/储存文件的地方.png)

本次demo使用向量数据库问pgvector，项目启动后，spring ai会连接pgvector自动创建vector_store表，当然也可以手动提前创建:
